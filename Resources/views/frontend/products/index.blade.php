                ...
                @if(Auth::user()->hasAccess('imonitor.alerts.index'))
                    <a href="{{ route('imonitor.alerts.index') }}">
                        ...
                        <span class="badge badge-danger" data-toggle="tooltip" data-placement="top"
                              title="Ver las alertas del producto" style="font-size: 1.5rem">
            ...
            <div class="row">
                @if(count($products))
                    <div class="col-12 mb-3">
                        ...
                        <div id="map_product" class="border-bottom border-primary bg-light"
                        ...
                        @forelse ($products  as $index => $product)
                            ...
                            <span class="d-block">
                                {{$product->created_at->format('m')}}/{{$product->created_at->format('m')}}
                                        ...
                                        <div class="col btn text-left text-truncate"
                                             onclick="centerMap({{$product->address}},{{$index}})">
                                            <h4>{{$product->title}}</h4>
                                        </div>
                                        <div class="col-auto">
                                            @if(Auth::user()->hasAccess('imonitor.alerts.index') && count($product->alersatives) > 0)
                                                <a class="btn btn-danger ml-1"
                                                   href="{{route('imonitor.alerts.product',$product->id)}}"
                                                   data-toggle="tooltip" data-placement="top"
                                                   title="Ver las alertas del producto">
                                    ...
                                    {{count($product->alersatives)}}
                                            ...
                                            <a class="btn btn-primary ml-1"
                                               href="{{ route('imonitor.product.show',$product->id) }}"
                                               data-toggle="tooltip" data-placement="top"
                                               title="Ir a la grafica de tiempo real">
                                                <i class="fa fa-area-chart text-white" aria-hidden="true"></i>
                                                <span class="d-none d-md-inline-block text-white">Tiempo Real</span>
                                            </a>
                                            <a class="btn btn-secondary ml-1"
                                               href="{{ route('imonitor.product.historic',$product->id) }}"
                                               data-toggle="tooltip" data-placement="top"
                                               title="Ir a la grafica del Histórico">
